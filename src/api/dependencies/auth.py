...
class RoleChecker:
    ...
    async def __call__(
        ...
        user_type = token_data.get("user_type", "CONSUMER")
...
def farmer_required():
    return RoleChecker([UserTypeEnum.FARMER])


def consumer_required():
    return RoleChecker([UserTypeEnum.CONSUMER])


def consumer_or_farmer_required():
    return RoleChecker([UserTypeEnum.CONSUMER, UserTypeEnum.FARMER])