...
class JWTGenerator:
    ...
    def generate_access_token(self, user: User) -> str:
        ...
        return self._generate_jwt_token(
            jwt_data=JWTUser(username=user.username, email=user.email).dict(),  # type: ignore
    ...
    def retrieve_details_from_token(self, token: str, secret_key: str) -> list[str]:
        try:
            ...
            jwt_user = JWTUser(username=payload["username"], email=payload["email"])
        ...
        return [jwt_user.username, jwt_user.email]
